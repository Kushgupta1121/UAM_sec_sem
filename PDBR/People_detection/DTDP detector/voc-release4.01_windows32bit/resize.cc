...
void alphacopy(double *src, double *dst, alphainfo *ofs, int n) 
{
  alphainfo *end = ofs + n;
  while (ofs != end)
  {
...
mxArray *resize(const mxArray *mxsrc, const mxArray *mxscale) {
  ...
  scale = mxGetScalar(mxscale);
  if (scale > 1)
    mexErrMsgTxt("Invalid scaling factor");   
    
  ddims[0] = (size_t)round(sdims[0]*scale);
  ddims[1] = (size_t)round(sdims[1]*scale);
  ddims[2] = sdims[2];
  ...
  resize1dtran(src, sdims[0], tmp, (int)ddims[0], sdims[1], sdims[2]);
  resize1dtran(tmp, sdims[1], dst, (int)ddims[1], (int)ddims[0], sdims[2]);
  mxFree(tmp);
