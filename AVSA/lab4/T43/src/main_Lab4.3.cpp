...
#define GRID_SIZE 4  //1,2,3,4,5,6... 			// grid_size (# candidates) can he changed here
#define STEP 3									// step size can be changed here
...
int main(int argc, char ** argv)
{
	...
	std::string output_path = "/home/avsa/eclipse-workspace/Lab4.3/outvideos/";								//location to save output videos
	//std::string dataset_path = "/home/avsa/eclipse-workspace/kush_lab4.2/src/datasets";
	// dataset paths
	std::string sequences[] = {"car1"										//test data for lab4.1b
	};
	/*
	...
	//------------------------------------------------------------------------------
	string feature = "GR"; 			// use GR for GRAY, RGB or HSV
	char clr_channel= 'G'; 			// If feature = RGB, then choose b/w R or G or B else is Feature= HSV then choose from H or S
	int histSize = 16;		 		// Number of bins can be modified here
	bool HOG_feature = false;    	// for HOG feature set this to true and select feature = GR, default value is false
	...
	for (int s=0; s<NumSeq; s++ )
	{
		...
		Rect model_Rect = list_bbox_gt[0];                // getting the bounding box for the model.
		Mat model_hist;
		...
		for (;;) {
			...
			// to initalize the model histogram extracting the model image from the frame
			...
			cvtColor(_model_image,gray_region_image,COLOR_BGR2GRAY);  // converting the model image to gray scale for processing in gray scale

			if (frame_idx == 1){  // model will be initalized only for the first frame

				if (feature == "RGB"){
					split(_model_image,channel);  // splitting the color image into RGB
					switch (clr_channel)
					{
					case 'B': // for Blue channel
						disp_img=channel[0];
						model_hist = displayHistogram(disp_img, histSize,feature,clr_channel); // calculating the histogram for blue channel
						//cout<< "B"<<endl;

						break ;
					case 'G': // For Green channel
						disp_img=channel[1];
						model_hist = displayHistogram(disp_img, histSize,feature,clr_channel);  // calculating the histogram for green channel
					//	cout<< "G model"<<endl;

						break;
					case 'R':  // for Red channel
						disp_img=channel[2];
						model_hist = displayHistogram(disp_img, histSize,feature,clr_channel);  // calculating the histogram for red channel
					//	cout<< "R"<<endl;
					...
					{	case 'H': // for Hue
						disp_img=channel[0];
						model_hist = displayHistogram(disp_img, histSize,feature,clr_channel);  // calculating the histogram for Hue
					//	cout << "H"<<endl;

						break;
					case 'S': // for saturation
						disp_img=channel[1];
						model_hist = displayHistogram(disp_img, histSize,feature,clr_channel);  // calculating the histogram for Saturation
					...
					model_hist = displayHistogram(disp_img, histSize,feature,clr_channel);  // calculating the histogram for gray feature
					//cout << "gray" << endl;
				}

				// HOG features will be calculated only for gray scale
				if(HOG_feature == true)  // to use HOG_features set this value to true (as mentioned above)
				{
					disp_img=gray_region_image;
			...
			// function to Compute the histogram of the candidates
