...
std::vector<Rect> Create_Candidates(int grid_size,  Rect prediction, int step, int Width, int Height)
	...
	for (int i=-grid_size; i<=grid_size; i++){
		...
		for (int j=-grid_size; j<=grid_size; j++){
			...
			candidates.push_back(Rect(prediction.x+(i*step),prediction.y+(j*step),Width,Height));
		}
...
std::vector<Mat> Compute_Candidates_Histograms(Mat frame,  vector<Rect> Candidates, int histSize ,string feature,char clr_channel, bool HOG_feature){
	...
	for (int i=0; i<Candidates.size(); i++){
		...
		if (Candidates[i].x >= 0 && Candidates[i].y >= 0 && Candidates[i].x + Candidates[i].width <= frame.cols && Candidates[i].y + Candidates[i].height <= frame.rows){
			...
			cand_image=frame(Candidates[i]);                           //(Range(y,y+Height),Range(x,x+Width));
			cand_image.copyTo(_cand_image);  // copying the region image to remove the green border
			cvtColor(cand_image,gray_region_image,COLOR_BGR2GRAY);


			if (feature == "RGB"){
				split(_cand_image,channel);
				switch (clr_channel)
				{
				case 'B': // for Blue channel
					disp_img=channel[0];
					cand_histogram = displayHistogram(disp_img, histSize,feature,clr_channel);
					cout<< "B"<<endl;
			...
			Candidates_histograms.push_back(cand_histogram);
...
std::vector<float> Compare_Model_Candidates(Mat model, vector<Mat> candidates_histogram)
	...
	for (int i=0; i<candidates_histogram.size(); i++){
		...
		double result = compareHist( model, candidates_histogram[i], CV_COMP_BHATTACHARYYA );
...
int Find_Lowest_Distance(vector<float> distances)
{
	...
	int Num_Candidate = std::min_element(distances.begin(),distances.end()) - distances.begin();
...
Mat computeHOG(Mat ROI,int histSize ) // Function to compute the HOG features
{
	...
	normalize(hog_img, hog_img, 0, hog_histImage.rows, NORM_MINMAX, -1, Mat() );

	return hog_img; // return the HOG feature Matrix

}

Mat displayHistogram(Mat channel,int histSize,string feature ,char clr) //Function to visualize the Histograms
{
	Mat hist;
