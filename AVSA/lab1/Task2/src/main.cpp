...
/*
 * Cambiar el dataset de input para que lea la imagenes de lite dataset
 * comprobar que se crean la carpetas y que los resultados se guardan donde queremos
 * Ejecutar todos los algoritmos y con los resultados ejecutar el código de evaluación en matlab
 * Hacer una tabla con cada resultado para cada task y para cada conjunto de parámetros
 * Comparar los resultados de kush y los míos
 */
...
//	string dataset_path = "/home/avsa/Desktop/P1/AVSA2020datasets/dataset2012lite/dataset"; //SET THIS DIRECTORY according to your download
...
// if (argc==3)	{input=argv[3]);}
	...
	string project_root_path = "/home/avsa/Desktop/Lab1"; //SET THIS DIRECTORY according to your project
	string project_name = "dataset2012lite"; // project exe name
	string results_path = project_root_path+"/"+project_name+"/results/Task2/Paremeter1";
	...
	for (int c=0; c<NumCat; c++ )
	{
		...
		for (int s=0; s<NumSeq; s++ )
		{
			...
			string inputvideo = dataset_path + "/" + dataset_cat[c] + "/" + baseline_seq[s] +"/input/" + image_path;
...
//			namedWindow("ForeGround Mask"); // final fgmask
			...
			double tau = 18; // to set ... //Lab1.1.1-3
			double alpha=0.2; // to set ... //Lab1.1.2-3
			bool selective_bkg_update = true; // true ... //Lab1.1.2-3
			int threshold_ghosts2=25; // to set ... //Lab1.1.3
			bool rgb = true; //true false;


			fgseg::bgs avsa_bgs(tau, alpha, selective_bkg_update, rgb, threshold_ghosts2); //construct object of the bgs class
			...
			Mat mat_counter;
			//= Mat::zeros(Size(img.cols,img.rows), CV_32S);

			vector<cv::String> fn;
			glob(dataset_path + "/" + dataset_cat[c] + "/" + baseline_seq[s] +"/input/*.jpg", fn, false);
			...
			for (int q=0;q<length;q++) {
				...
				if (it==1)
					 ...
					 //Mat mat_counter = Mat::zeros(Size(img.cols,img.rows), CV_32S);
				 	 }

				if (it==1 && rgb ==false){

					mat_counter = Mat::zeros(Size(img.cols,img.rows), CV_32S);
				}

				if (it==1 && rgb ==true){

					mat_counter = Mat::zeros(Size(img.cols,img.rows), CV_8UC3);
				}
		        ...
		        //...
           		...
           		//HERE YOU HAVE TO CHOOSE WHICH TASK YOU WANT TO CHECK
           		//UNCOMMENT DEPENDING THE TASK

           		//Lab1.1.1
           		//avsa_bgs.bkgSubtraction(img);

           		//Lab1.1.2
           		avsa_bgs.bkgSubtraction_Lab112(img);

           		//Lab1.1.3
           		//mat_counter = avsa_bgs.bkgSubtraction_Lab113(img, mat_counter);
